...
exports.createSkinHealthIndex = async (req, res) => {
    try {
        ...
        const { title, description, image, article_content, article_type, status, screen_image, screen_image_title, content_type } = req.body;
...
exports.updateSkinHealthIndex = async (req, res) => {
    try {
        ...
        if (updateData.article_type === "Common skin conditions" || updateData.article_type === "Skin signs of health") {
            const articleTypeCheck = await Article.findOne({ where: { article_type: updateData.article_type, id: id } });

            if (!articleTypeCheck) {
                return res.status(200).json({ message: `This id doesn't belong to ${updateData.article_type} article`, data: [] });
            }
            ...
            return res.status(400).json({ message: "No such article found with this name", data: [] });
...
exports.deleteSkinHealthIndex = async (req, res) => {
    try {
        ...
        if (articleType === "Common skin conditions" || articleType === "Skin signs of health") {
            articleModel = Article;
        } else {
            return res.status(400).json({ message: 'Invalid article_type', data: [] });
        }

        const article = await articleModel.findOne({ where: { id, article_type: articleType } });
...
exports.getSkinHealthIndex = async (req, res) => {
    try {
        const skinCondition = await Article.findAll({ where: { article_type: "Common skin conditions" } });
        const skinSigns = await Article.findAll({ where: { article_type: "Skin signs of health" } });

        const skinRoutine101 = await getDataRoutine101(
            await Article.findAll({ where: { article_type: "Skincare Routine 101" } })
        );
        ...
        res.status(200).json({
            data: {
                skinCondition: await getDataWithImages(skinCondition),
                skinSigns: await getDataWithImages(skinSigns),
                skinRoutine101,
            }
        });
    } catch (err) {
...
exports.getSkinHealthIndexById = async (req, res) => {
    try {
        ...
        const articleImages = await ArticleImage.findAll({ where: { article_id: id }, attributes: { exclude: ['createdAt', 'updatedAt'] } });
